...
\begin{document}
	...
	In addition to the common function definition, e.g. $f(x, y) = x^2 + y^2$, a function can also be defined in-line with the mapping arrow $\mapsto$, e.g. $(x, y) \mapsto x^2 + y^2$.
	
	Given $f : A \to B$ and $g : B \to C$, the function composition $g \circ f : A \to C$ denotes the chaining of $g$ after $f$, i.e. $x \mapsto g(f(x))$.
	...
	\begin{defn}
 	...
 	\begin{defn}
 		An \emph{equivalence} relation is
 	\end{defn}
